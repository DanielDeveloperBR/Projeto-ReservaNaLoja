...
function eventoCep(cepAtual) {
  cepAtual.addEventListener('input', async () => {
    ...
    try {
      ...
      const response = await fetch(`https://backend-express-mu.vercel.app/cep/${cep}/`, {
        method: 'GET',
        headers: {
...
formularios.querySelector('#formEmpresa').addEventListener("submit", async (event) => {
  ...
  fetch('https://backend-express-mu.vercel.app/empresa', {
    method: 'POST',
    body: formData,
  })
    .then(async response => {
...
formularios.querySelector('#formCliente').addEventListener("submit", (event) => {
  ...
  formData.append('estado', estado.value);
  formData.append('imagemPerfil', imagemPerfil);
  fetch('https://backend-express-mu.vercel.app/usuario', {
    method: 'POST',
    body: formData,
  })
    .then(async response => {
